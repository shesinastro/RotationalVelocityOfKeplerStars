...
#calculate rms of the spectrum in flat region, that serves as estimate of the error
counts_target_array = np.asarray(target['N_target'])
ll_target_array = np.asarray(target['l_target'])
counts_target_array_selected = counts_target_array[(ll_target_array > 6525.) & (ll_target_array < 6550)]
mean_count = np.mean(counts_target_array_selected)
rms = np.sqrt(np.sum((counts_target_array_selected-mean_count)**2)/len(counts_target_array_selected))
...
#errors_target = np.random.normal(size=len(ll_target),scale=rms)
errors_target = np.repeat(rms, len(ll_target)) #error is equal to rms of the flat part of the spectrum

#pl.plot(ll_target, counts_target)
#pl.plot(ll_template, counts_template)
#pl.show()
#pl.close()
...
for i in range(0, 99, 1):
    rv, cc = pyasl.crosscorrRV(bootres[i][:,0], bootres[i][:,1], ll_template_temp, counts_template_temp, -100., 100., 1.,skipedge=20)
    maxind = np.argmax(cc) 
...
f=open('vsini.dat', 'a')
#f.write("""{0} {1} {2[0]} +{2[1]} -{2[2]'\n'}""".format(KOI, vsini_DEF, vsini_mcmc))
f.write('%d\t%f\t%f\t%f\t%f\n' % (KOI, vsini_DEF, vsini_mcmc[0], vsini_mcmc[1],vsini_mcmc[2]))
